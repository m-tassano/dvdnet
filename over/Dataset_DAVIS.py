...
class Dataset_DAVIS(torch.utils.data.Dataset):
    ...
    def __init__(self, root_dir, temp_psz=5, transform=None, noise_sigma=0.1, device='cpu'):
        """
        Args:
        ...
        self.list_videoFolders = sorted(glob.glob(path2videos+"/*"))
    ...
    def __getitem__(self, idx):
        ...
        inframes = torch.from_numpy(inframes[:, np.newaxis, :, :, :]).to(self.device)
        # target
...
if __name__=="__main__":
    ...
    inframes, target, noisestd = dataset[5]
    print(len(inframes))
